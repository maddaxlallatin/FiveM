...
Citizen.CreateThread(function()
    ...
    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
               ...
               if (vehicleClass == 18 and bro == true) then 
                speedlimiter = true
                SetVehicleMaxSpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false), meterspeed)
               
               end

               if (IsDisabledControlJustPressed(0, 85)) then
                if speedlimiter == true then
                    SetVehicleMaxSpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false), 89.4)
                    speedlimiter = false
                    bro = false
                    Notify('~r~Speed Limiter Toggled')
